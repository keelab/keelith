...
// RuntimeConfig declares one MySQL transaction-local Inbox.
...
func ValidateRuntimeConfig(config RuntimeConfig) error {
	...
	if _, err := inboxIsolationLevel(config.Isolation); err != nil {
		return err
	}
...
// Runtime binds a MySQL Executor to stable consumer and retry semantics.
type Runtime struct {
	executor   *Executor
	consumer   string
...
// NewRuntime constructs an Inbox runtime without starting background work.
func NewRuntime(
	...
	isolation, err := inboxIsolationLevel(config.Isolation)
	if err != nil {
		return nil, err
	}
...
func inboxIsolationLevel(value string) (sql.IsolationLevel, error) {
