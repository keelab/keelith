...
// RuntimeConfig declares one PostgreSQL-backed Outbox Dispatcher.
...
// ValidateRuntimeConfig rejects unbounded or inconsistent dispatcher state.
...
// Runtime owns the Repository used by business transactions and Dispatcher.
type Runtime struct {
	repository *Repository
	dispatcher *coreoutbox.Dispatcher
}

// NewRuntime composes PostgreSQL persistence with the broker-neutral
// Dispatcher while preserving one Repository for transactional Enqueue.
func NewRuntime(
	...
	return &Runtime{repository: repository, dispatcher: dispatcher}, nil
}
