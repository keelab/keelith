...
const (
	maxInboxRetentionBatchSize = 10_000
	maxInboxRetentionBatches   = 100
)

// RetentionConfig bounds one scheduled Inbox retention execution.
...
// ValidateRetentionConfig rejects unsafe replay horizons and unbounded work.
func ValidateRetentionConfig(config RetentionConfig) error {
	if config.ProcessedRetention <= 0 ||
		config.BatchSize <= 0 ||
		config.BatchSize > maxInboxRetentionBatchSize ||
		config.MaxBatches <= 0 ||
		config.MaxBatches > maxInboxRetentionBatches ||
		config.RetryAfter <= 0 {
...
// NewRetentionRuntime composes scheduled cleanup with the exact Inbox
// Executor used by transactional message processing.
